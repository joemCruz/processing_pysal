...
class MarkovSpatial(GeoAlgorithm):
    ...
    def processAlgorithm(self, progress):
        ...
        if contiguity == 0: # 0 for queen
            print 'INFO: Spatial Markov using queen contiguity'
            w = pysal.queen_from_shapefile(filename)
        else: # 1 for rook
            print 'INFO: Spatial Markov using rook contiguity'
        ...
        m = pysal.Spatial_Markov(pci.transpose(), w, fixed = True, k = 5,
            variable_name = 'spatialMarkovVar')
        print ("INFO: Spatial Markov")
        print ("INFO: A-Spatial Measures")
        print ("A-Spatial Transitions: \n {0}".format(m.transitions))
        print ("A-Spatial Steady State Matrix: \n {0}".format(m.s))
        print ("P-Matrix: \n {0}".format(m.p))
        print ("INFO: Spatial Measures:")
        print ("Spatial Transitions: \n {0}".format(m.T))
        print ("Spatial Steady State Matrix: \n {0}".format(m.S))
        print ("Spatial P-Matrix: \n {0}".format(m.P))
